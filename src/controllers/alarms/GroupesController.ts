...
exports.list = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    try {
      let all = await prisma.alarm_groups.findMany({
        orderBy: { id : "asc" },
        include: {
          companies: true,
        },
...
exports.list_count = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    try {
      const count = await prisma.alarm_groups.count();
...
exports.list_lazy = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    // Process request
    try {
      const result = await prisma.alarm_groups.findMany({
        skip:
...
exports.list_lazy_count = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const all = await prisma.alarm_groups.count({
        where: whereClause,
        orderBy: sortingClause,
      });
      ...
      if (all) {
        ...
        return response.status(400).json("groupes is empty");
      }
    } catch (error: any) {
      return response.status(500).json(error.message);
    }
...
exports.detail = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const byId = await prisma.alarm_groups.findUnique({
        where: {
          id: id,
        },
        include: {
      ...
      if (byId) {
        ...
        return response
          .status(400)
          .json("catalog groupes with id(" + id + ") not found");
      }
    } catch (error: any) {
      return response.status(500).json(error.message);
    }
...
exports.create_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    // check duplicates
    const existing = await prisma.alarm_groups.findFirst({
      where: {
    ...
    try {
      ...
      const catalogResult = await prisma.alarm_groups.create({
        data: {
          ...catalog,
        },
      });
      ...
      console.log("GroupesController create_post", error.message);
      return response.status(500).json(error.message);
    }
  }
);
...
exports.update_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      ...
      const catalogResult = await prisma.alarm_groups.update({
        where: { id: id },
        data: {
          ...catalog,
        },
      ...
      console.log("GroupesController update_post", error.message);
...
exports.delete_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const catalogResult = await prisma.alarm_groups.delete({
        where: { id: id },
      });
...
exports.download_lazy = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    const filename =
      request.params.filename || "groupes_" + Math.floor(Date.now() / 1000);
    const fields = prisma.alarm_groups.fields;
