...
exports.list = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    try {
      let all = await prisma.analyse_types.findMany({
        orderBy: { id : "asc" },
        include: {
          businesses: true,
          analyse_categories: true,
          companies: true,
          analyse_methods: true,
          meas_units: true,
        },
      });
...
exports.list_count = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    try {
      const count = await prisma.analyse_types.count();
...
exports.list_lazy = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    // Process request
    try {
      const result = await prisma.analyse_types.findMany({
        skip:
...
exports.list_lazy_count = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const all = await prisma.analyse_types.count({
        where: whereClause,
        orderBy: sortingClause,
      });
...
exports.detail = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const byId = await prisma.analyse_types.findUnique({
        where: {
          id: id,
        },
        include: {
...
exports.create_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    // check duplicates
    const existing = await prisma.analyse_types.findFirst({
      where: {
    ...
    try {
      ...
      const catalogResult = await prisma.analyse_types.create({
        data: {
          ...catalog,
        },
      });
      ...
      console.log("AnalyseTypesController create_post", error.message);
      return response.status(500).json(error.message);
    }
  }
);
...
exports.update_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      ...
      const catalogResult = await prisma.analyse_types.update({
        where: { id: id },
        data: {
          ...catalog,
        },
      ...
      console.log("AnalyseTypesController update_post", error.message);
...
exports.delete_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const catalogResult = await prisma.analyse_types.delete({
        where: { id: id },
      });
...
exports.download_lazy = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    const filename =
      request.params.filename || "analyseTypes_" + Math.floor(Date.now() / 1000);
    const fields = prisma.analyse_types.fields;
