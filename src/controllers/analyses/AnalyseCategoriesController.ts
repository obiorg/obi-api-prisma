...
exports.list = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    try {
      let all = await prisma.analyse_categories.findMany({
        orderBy: { id: "asc" },
        include: {
          businesses: true,
          companies: true,
...
exports.list_count = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    try {
      const count = await prisma.analyse_categories.count();
...
exports.list_lazy = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    // Process request
    try {
      const result = await prisma.analyse_categories.findMany({
        skip:
...
exports.list_lazy_count = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const all = await prisma.analyse_categories.count({
        where: whereClause,
        orderBy: sortingClause,
      });
...
exports.detail = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const byId = await prisma.analyse_categories.findUnique({
        where: {
          id: id,
        },
        include: {
...
exports.create_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    // check duplicates
    const existing = await prisma.analyse_categories.findFirst({
      where: {
    ...
    if (existing) {
      const error = {
        errors: {
          ...
          category: ["Doublon ! ...déjà spécifié !"],
        },
      };
    ...
    try {
      ...
      const catalogResult = await prisma.analyse_categories.create({
        data: {
          ...catalog,
        },
      });
      ...
      console.log("AnalyseCategoriesController create_post", error.message);
      return response.status(500).json(error.message);
    }
  }
);
...
exports.update_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      ...
      const catalogResult = await prisma.analyse_categories.update({
        where: { id: id },
        data: {
          ...catalog,
        },
      ...
      console.log("AnalyseCategoriesController update_post", error.message);
...
exports.delete_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const catalogResult = await prisma.analyse_categories.delete({
        where: { id: id },
      });
...
exports.download_lazy = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    const filename =
      ...
      "analyseCategories_" + Math.floor(Date.now() / 1000);
    const fields = prisma.analyse_categories.fields;
