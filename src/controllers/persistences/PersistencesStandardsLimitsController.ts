...
exports.list = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    try {
      let all = await prisma.pers_standard_limits.findMany({
        orderBy: { id : "asc" },
        include: {
          companies: true,
          meas_comparators: true,
          meas_limits_groups: true,
          pers_standard_pers_standard_limits_pers_standardTopers_standard: true,
          tags: true,
        },
      });
...
exports.list_count = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    try {
      const count = await prisma.pers_standard_limits.count();
...
exports.list_lazy = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    // Process request
    try {
      const result = await prisma.pers_standard_limits.findMany({
        skip:
...
exports.list_lazy_count = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const all = await prisma.pers_standard_limits.count({
        where: whereClause,
        orderBy: sortingClause,
      });
...
exports.detail = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const byId = await prisma.pers_standard_limits.findUnique({
        where: {
          id: id,
        },
        include: {
...
exports.create_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    // check duplicates
    const existing = await prisma.pers_standard_limits.findFirst({
      where: {
        company: request.body.company,
        tag: request.body.tag,
        pers_standard: request.body.pers_standard,
    ...
    if (existing) {
      const error = {
        errors: {
          ...
          pers_standard: ["Doublon ! ...déjà spécifié !"],
    ...
    try {
      ...
      const catalogResult = await prisma.pers_standard_limits.create({
        data: {
          ...catalog,
        },
      });
      ...
      console.log("PersistencesLimitsStandardsController create_post", error.message);
      return response.status(500).json(error.message);
    }
  }
);
...
exports.update_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      ...
      const catalogResult = await prisma.pers_standard_limits.update({
        where: { id: id },
        data: {
          ...catalog,
        },
      ...
      console.log("PersistencesLimitsStandardsController update_post", error.message);
...
exports.delete_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const catalogResult = await prisma.pers_standard_limits.delete({
        where: { id: id },
      });
...
exports.download_lazy = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    const filename =
      request.params.filename || "pers_standard_limits_" + Math.floor(Date.now() / 1000);
    const fields = prisma.pers_standard_limits.fields;
