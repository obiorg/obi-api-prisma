...
exports.list = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    try {
      let all = await prisma.user_login_data.findMany({
        orderBy: { id : "asc" },
        include: {
          user_email_verified: true,
          user_hashing_algorithms: true,
        },
      });
...
exports.list_count = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    try {
      const count = await prisma.user_login_data.count();
...
exports.list_lazy = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    // Process request
    try {
      const result = await prisma.user_login_data.findMany({
        skip:
...
exports.list_lazy_count = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const all = await prisma.user_login_data.count({
        where: whereClause,
        orderBy: sortingClause,
      });
...
exports.detail = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const byId = await prisma.user_login_data.findUnique({
        where: {
          id: id,
        },
        include: {
...
exports.create_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    // check duplicates
    const existing = await prisma.user_login_data.findFirst({
      where: {
        loginName: request.body.loginName,
        email: request.body.email,
        emailVerified: request.body.emailVerified,
        hashAlgorithm: request.body.hashAlgorithm,
      },
    });
    if (existing) {
      const error = {
        errors: {
          loginName: ["Doublon ! ...déjà spécifié !"],
          email: ["Doublon ! ...déjà spécifié !"],
          emailVerified: ["Doublon ! ...déjà spécifié !"],
          hashAlgorithm: ["Doublon ! ...déjà spécifié !"],
        },
      };
    ...
    try {
      ...
      const catalogResult = await prisma.user_login_data.create({
        data: {
          ...catalog,
        },
      });
      ...
      console.log("UserLoginDataController create_post", error.message);
      return response.status(500).json(error.message);
    }
  }
);
...
exports.update_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    if (!existing) {
      const error = {
        errors: {
          loginName: ["... n'existe plus !"],
          email: ["... n'existe plus !"],
          emailVerified: ["... n'existe plus !"],
          hashAlgorithm: ["... n'existe plus !"],
        },
      };
      return response.status(400).json(error);
    }
    ...
    try {
      ...
      const catalogResult = await prisma.user_login_data.update({
        where: { id: id },
        data: {
          ...catalog,
        },
      ...
      console.log("UserLoginDataController update_post", error.message);
...
exports.delete_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const catalogResult = await prisma.user_login_data.delete({
        where: { id: id },
      });
...
exports.download_lazy = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    const filename =
      request.params.filename || "userLoginData_" + Math.floor(Date.now() / 1000);
    const fields = prisma.user_login_data.fields;
