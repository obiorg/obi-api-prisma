...
exports.list = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    try {
      let all = await prisma.user_email_verified.findMany({
        orderBy: { id : "asc" },
      });
...
exports.list_count = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    try {
      const count = await prisma.user_email_verified.count();
...
exports.list_lazy = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    // Process request
    try {
      const result = await prisma.user_email_verified.findMany({
        skip:
...
exports.list_lazy_count = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const all = await prisma.user_email_verified.count({
        where: whereClause,
        orderBy: sortingClause,
      });
...
exports.detail = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const byId = await prisma.user_email_verified.findUnique({
        where: {
          id: id,
        },
      });
...
exports.create_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    // check duplicates
    const existing = await prisma.user_email_verified.findFirst({
      where: {
        statusDescription: request.body.statusDescription,
      },
    });
    if (existing) {
      const error = {
        errors: {
          statusDescription: ["Doublon ! ...déjà spécifié !"],
        },
      };
    ...
    try {
      ...
      const catalogResult = await prisma.user_email_verified.create({
        data: {
          ...catalog,
        },
      });
      ...
      console.log("UserEmailVerifiedController create_post", error.message);
      return response.status(500).json(error.message);
    }
  }
);
...
exports.update_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    if (!existing) {
      const error = {
        errors: {
          statusDescription: ["... n'existe plus !"],
        },
      };
      return response.status(400).json(error);
    }
    ...
    try {
      ...
      const catalogResult = await prisma.user_email_verified.update({
        where: { id: id },
        data: {
          ...catalog,
        },
      ...
      console.log("UserEmailVerifiedController update_post", error.message);
...
exports.delete_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      const catalogResult = await prisma.user_email_verified.delete({
        where: { id: id },
      });
...
exports.download_lazy = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    const filename =
      request.params.filename || "userEmailVerified_" + Math.floor(Date.now() / 1000);
    const fields = prisma.user_email_verified.fields;
