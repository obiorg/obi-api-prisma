  ...
  async (request: Request, response: Response, next: any) => {
    ...
    const existing = await prisma.tags_lists.findFirst({
      where: {
        ...
        type: request.body.company,
        list: request.body.company,
      },
    });
    if (existing) {
      const error = {
    ...
    try {
      ...
      console.log("TagsListController create_post", error.message);
      return response.status(500).json(error.message);
    }
  }
);
...
exports.update_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    try {
      ...
      console.log("TagsListController update_post", error.message);
