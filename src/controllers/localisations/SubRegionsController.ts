...
exports.list = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    try {
      const all = await prisma.loc_subregions.findMany({});
...
exports.create_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    //   const newEntity = await prisma.loc_subregions.create(entity);
    ...
    return response
      .status(400)
      .json({ errors: "loc_subregions create not implemented !" });
  }
);
...
exports.update_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    //     // try {
    //     //   const entity = request.body;
    //     //   const updateEntity = await prisma.loc_subregions.update(entity, id);
...
exports.delete_post = asyncHandler(
  async (request: Request, response: Response, next: any) => {
    ...
    // try {
    //   await prisma.loc_subregions.delete(id);
    //   return response.status(204).json("loc_subregions has been successfully deleted");
    // } catch (error: any) {
    //   return response.status(500).json(error.message);
    // }
    return response.status(400).json({ errors: "No implemented !" });
